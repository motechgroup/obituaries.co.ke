...
<section class="relative w-full h-[500px] min-h-[380px] flex items-end overflow-hidden border-b border-surface-container-high">
    ...
    <div class="relative z-10 w-full max-w-[1200px] mx-auto px-6 pb-8 flex flex-col md:flex-row items-center md:items-start gap-8">
        <div class="relative -mb-16 md:mb-0 flex-shrink-0">
            <div class="w-44 h-44 md:w-52 md:h-52 rounded-full border-8 border-background overflow-hidden shadow-2xl bg-surface-container">
                @if($obituary->photo)
                ...
                @else
                    <div class="w-full h-full bg-gradient-to-b from-primary to-primary-container flex flex-col items-center justify-center p-4 text-center text-on-primary">
                        <span class="material-symbols-outlined text-[48px] text-secondary-fixed mb-1">person</span>
        ...
        <div class="flex flex-col items-center md:items-start text-center md:text-left pt-2">
            <span class="text-xs font-bold text-secondary tracking-[0.2em] uppercase mb-2">In Loving Memory</span>
            <h1 class="font-serif text-3xl sm:text-5xl font-bold text-primary mb-2">{{ $obituary->full_name }}</h1>
            <div class="flex items-center gap-3 text-on-surface-variant mb-3 font-semibold text-sm">
            ...
            <p class="font-serif italic text-on-surface-variant opacity-80 text-sm">"Forever in our hearts"</p>
        </div>
    </div>
</section>

<!-- Content Grid from Stitch Design -->
<div class="max-w-[1200px] mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-12 mt-16 pb-20">
    
    <!-- Left Column: Biography & Life Story -->
    <div class="lg:col-span-8 space-y-12">
        <section class="relative">
            <h2 class="font-serif text-2xl sm:text-3xl font-bold text-primary mb-6 flex items-center gap-4">
                The Life & Journey
        ...
        <div class="p-6 bg-surface-container-low rounded-xl border-l-4 border-secondary text-xs text-on-surface-variant flex items-center justify-between">
            <div>
                <span class="font-bold text-primary block text-sm">Submitted with love by {{ $obituary->submitter_name }}</span>
                <span>Relationship: {{ $obituary->relationship }}</span>
            </div>
            <span class="font-serif italic text-secondary font-bold">Obituaries.co.ke</span>
        </div>
    ...
    <div class="lg:col-span-4 space-y-8" x-data="{ copied: false }">
        
        <!-- Funeral Service Info Card -->
        <div class="sticky top-28 bg-surface-container-lowest rounded-2xl shadow-md p-6 sm:p-8 border border-surface-container space-y-6">
            ...
            <div class="space-y-5 text-xs">
                ...
                <div class="flex gap-4 items-start">
                    ...
                    <div>
                        ...
                        <p class="font-semibold text-on-surface text-sm">{{ $obituary->town }}, {{ $obituary->county }} County</p>
                    </div>
                </div>
                ...
                @if($obituary->funeral_date)
                    <div class="flex gap-4 items-start">
                        ...
                        <div>
                            ...
                            <p class="font-semibold text-on-surface text-sm">{{ $obituary->funeral_date->format('l, F d, Y') }}</p>
                        </div>
                    </div>
                @endif
                ...
                @if($obituary->church_service_location)
                    <div class="flex gap-4 items-start">
                        ...
                        <div>
                            ...
                            <p class="font-semibold text-on-surface text-sm">{{ $obituary->church_service_location }}</p>
                        </div>
                    </div>
                @endif
                ...
                @if($obituary->burial_location)
                    <div class="flex gap-4 items-start">
                        ...
                        <div>
                            ...
                            <p class="font-semibold text-on-surface text-sm">{{ $obituary->burial_location }}</p>
                        </div>
                    </div>
                @endif
            </div>
            ...
            @if($obituary->programme_file)
                <a href="{{ asset('storage/' . $obituary->programme_file) }}" target="_blank" class="w-full mt-6 flex items-center justify-center gap-2 bg-primary text-on-primary py-3.5 rounded-xl text-xs font-semibold hover:bg-primary-container transition-all shadow-sm">
