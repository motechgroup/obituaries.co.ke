...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div class="bg-white border border-slate-200 p-5 rounded-2xl space-y-1 shadow-sm">
            <span class="text-[10px] font-extrabold uppercase tracking-wider text-slate-500 block">Today's Revenue</span>
            <div class="text-xl sm:text-2xl font-black text-emerald-700 font-mono">KES {{ number_format($todayRevenue) }}</div>
        </div>
        <div class="bg-white border border-slate-200 p-5 rounded-2xl space-y-1 shadow-sm">
            <span class="text-[10px] font-extrabold uppercase tracking-wider text-slate-500 block">Weekly Revenue</span>
            <div class="text-xl sm:text-2xl font-black text-emerald-700 font-mono">KES {{ number_format($weeklyRevenue) }}</div>
        </div>
        <div class="bg-white border border-slate-200 p-5 rounded-2xl space-y-1 shadow-sm">
            <span class="text-[10px] font-extrabold uppercase tracking-wider text-slate-500 block">Monthly Revenue</span>
            <div class="text-xl sm:text-2xl font-black text-emerald-700 font-mono">KES {{ number_format($monthlyRevenue) }}</div>
        </div>
        <div class="bg-white border border-slate-200 p-5 rounded-2xl space-y-1 shadow-sm">
            <span class="text-[10px] font-extrabold uppercase tracking-wider text-slate-500 block">Annual Revenue</span>
            <div class="text-xl sm:text-2xl font-black text-amber-800 font-mono">KES {{ number_format($annualRevenue) }}</div>
        </div>
        <div class="bg-white border border-slate-200 p-5 rounded-2xl space-y-1 shadow-sm">
            <span class="text-[10px] font-extrabold uppercase tracking-wider text-slate-500 block">Outstanding</span>
            <div class="text-xl sm:text-2xl font-black text-rose-700 font-mono">KES {{ number_format($outstandingPayments) }}</div>
        </div>
    </div>
            ...
            <div class="space-y-3 divide-y divide-slate-100">
                @foreach($revenueByPlacement as $rp)
                    <div class="flex items-center justify-between text-xs pt-2">
                        <span class="text-slate-900 font-bold">{{ $rp->name }}</span>
                        <span class="font-mono text-emerald-700 font-extrabold">KES {{ number_format($rp->total) }}</span>
                    </div>
                @endforeach
            </div>
        </div>
        ...
        <div class="bg-white border border-slate-200 rounded-3xl p-6 space-y-4 shadow-sm">
            ...
            <div class="space-y-3 divide-y divide-slate-100">
                @foreach($revenueByCategory as $rc)
                    <div class="flex items-center justify-between text-xs pt-2">
                        <span class="text-slate-900 font-bold">{{ $rc->name ?: 'General Funeral Services' }}</span>
                        <span class="font-mono text-emerald-700 font-extrabold">KES {{ number_format($rc->total) }}</span>
                    </div>
                @endforeach
            </div>
        </div>
            ...
            <table class="w-full text-left text-xs text-slate-700">
                <thead class="bg-slate-50 text-slate-700 font-bold uppercase tracking-wider border-b border-slate-200">
                    <tr>
                        ...
                        <th class="py-3.5 px-4">M-Pesa Phone</th>
                        <th class="py-3.5 px-4">Amount</th>
                        <th class="py-3.5 px-4">Status</th>
                        <th class="py-3.5 px-4 text-right">Paid Date</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-200">
                    @foreach($recentPayments as $pay)
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="py-3.5 px-4 text-right text-slate-600 font-mono font-medium">
