...
@section('title', 'Register New Advertiser Account | Admin Panel')

@section('content')
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center justify-between border-b border-slate-200 pb-4">
        <div>
            <h1 class="font-serif text-2xl sm:text-3xl font-bold text-slate-900">Register New Advertiser Account</h1>
            <p class="text-xs sm:text-sm font-medium text-slate-600">Create an advertiser account for a funeral home or corporate client.</p>
        </div>
    ...
    <form action="{{ route('admin.advertising.advertisers.store') }}" method="POST" class="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 shadow-sm space-y-6">
        @csrf
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <input type="email" name="email" value="{{ old('email') }}" required placeholder="info@leefuneral.co.ke" class="w-full px-4 py-3 bg-slate-50 border border-slate-300 rounded-xl text-xs sm:text-sm font-medium text-slate-900 focus:bg-white focus:border-amber-500 outline-none transition-all">
            </div>
            ...
            <div>
                ...
                <input type="password" name="password" required placeholder="Minimum 8 characters" class="w-full px-4 py-3 bg-slate-50 border border-slate-300 rounded-xl text-xs sm:text-sm font-medium text-slate-900 focus:bg-white focus:border-amber-500 outline-none transition-all">
            </div>
            ...
            <div>
                ...
                <select name="status" required class="w-full px-4 py-3 bg-slate-50 border border-slate-300 rounded-xl text-xs sm:text-sm font-bold text-slate-900 focus:bg-white focus:border-amber-500 outline-none transition-all">
                    <option value="active" {{ old('status') === 'active' ? 'selected' : '' }}>Active Account (Can log in & manage ads)</option>
                    <option value="suspended" {{ old('status') === 'suspended' ? 'selected' : '' }}>Suspended Account (Blocked from login)</option>
                </select>
            </div>
        </div>
        ...
        <div class="pt-4 border-t border-slate-100 flex items-center justify-end space-x-3">
            ...
            <button type="submit" class="px-6 py-3 bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold rounded-xl text-xs uppercase tracking-wider transition-all shadow-md">
                Register Advertiser Account &rarr;
            </button>
        </div>
    </form>
</div>
