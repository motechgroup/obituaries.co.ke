    ...
    <form action="{{ route('admin.settings.update') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div x-show="activeTab === 'branding'" class="bg-white rounded-2xl p-6 sm:p-8 border border-slate-200 shadow-xs space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <!-- Google Analytics Integration (No Card) -->
                <div class="sm:col-span-2 space-y-1.5">
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-800">Google Analytics (GA4) Tag / Measurement ID</label>
                    <input type="text" name="google_analytics_measurement_id" value="{{ old('google_analytics_measurement_id', $settings['google_analytics_measurement_id']) }}" placeholder="e.g. G-ABC123XYZ or <script>...</script>" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-sm font-mono">
                    <p class="text-xs text-slate-500">
                        Enter your Google Analytics 4 Measurement ID (e.g. <code class="bg-slate-100 px-1.5 py-0.5 rounded border border-slate-200">G-XXXXXXXXXX</code>) or paste the full <code class="bg-slate-100 px-1.5 py-0.5 rounded border border-slate-200">&lt;script&gt;</code> tag. It will automatically inject Google Tag code into the website.
                    </p>
                </div>
