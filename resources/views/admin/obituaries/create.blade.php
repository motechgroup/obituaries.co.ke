...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-2xl p-6 sm:p-8 border border-slate-200 shadow-xs">
        <form action="{{ route('admin.obituaries.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="space-y-4 pt-2">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="date_of_birth" value="{{ old('date_of_birth') }}" placeholder="e.g. 1945 or 15/04/1945" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-sm">
                    ...
                    <div>
                        ...
                        <input type="text" name="date_of_death" value="{{ old('date_of_death') }}" required placeholder="e.g. 2026 or 28/07/2026" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-sm">
                    ...
                    <div class="sm:col-span-2" x-data="{ content: '', previewMode: false }">
                        ...
                        <div class="bg-slate-100 border border-slate-300 border-b-0 rounded-t-xl p-2 flex flex-wrap items-center gap-1.5 text-xs select-none">
                            <button type="button" @mousedown.prevent @click="window.applyTag('b', 'admin_biography')" class="px-2.5 py-1 bg-white hover:bg-slate-200 border border-slate-300 rounded font-bold text-slate-900 shadow-2xs" title="Bold Text">B</button>
                            <button type="button" @mousedown.prevent @click="window.applyTag('i', 'admin_biography')" class="px-2.5 py-1 bg-white hover:bg-slate-200 border border-slate-300 rounded italic text-slate-900 shadow-2xs" title="Italic Text">I</button>
                            <button type="button" @mousedown.prevent @click="window.applyTag('u', 'admin_biography')" class="px-2.5 py-1 bg-white hover:bg-slate-200 border border-slate-300 rounded underline text-slate-900 shadow-2xs" title="Underline Text">U</button>
                            <span class="w-[1px] h-5 bg-slate-300 mx-1"></span>
                            <button type="button" @mousedown.prevent @click="window.applyTag('h3', 'admin_biography')" class="px-2.5 py-1 bg-white hover:bg-slate-200 border border-slate-300 rounded font-bold text-amber-800 shadow-2xs" title="Section Heading">Heading 3</button>
                            <button type="button" @mousedown.prevent @click="window.applyTag('p', 'admin_biography')" class="px-2.5 py-1 bg-white hover:bg-slate-200 border border-slate-300 rounded text-slate-800 shadow-2xs" title="Paragraph">&lt;p&gt; Para</button>
                            <button type="button" @mousedown.prevent @click="window.applyList('admin_biography')" class="px-2.5 py-1 bg-white hover:bg-slate-200 border border-slate-300 rounded text-slate-800 shadow-2xs" title="Bullet List">&bull; Bullet List</button>
                            <button type="button" @mousedown.prevent @click="window.applyTag('blockquote', 'admin_biography')" class="px-2.5 py-1 bg-white hover:bg-slate-200 border border-slate-300 rounded italic text-slate-700 shadow-2xs" title="Quote">“Quote”</button>
                            <span class="w-[1px] h-5 bg-slate-300 mx-1"></span>
                            <button type="button" @mousedown.prevent @click="previewMode = !previewMode" class="ml-auto px-3 py-1 bg-slate-800 text-white rounded font-bold text-[11px] shadow-2xs flex items-center space-x-1" x-text="previewMode ? '✍️ Edit Text' : '👁️ Live Formatted Preview'"></button>
                        </div>
            ...
            @if(Auth::guard('admin')->user()->isEditor())
                <!-- M-Pesa Payment Code for Editors -->
                <div class="p-5 bg-amber-50/80 border border-amber-300 rounded-2xl space-y-3">
                    <div class="flex items-center space-x-2 text-amber-950 font-bold text-sm">
                        <span class="material-symbols-outlined text-[20px] text-amber-700">payments</span>
                        <span>M-Pesa Payment Verification Code <span class="text-rose-600">* (Required for Editors)</span></span>
                    </div>
                    <p class="text-xs text-amber-900 leading-relaxed">
                        Editors cannot post free notices. Enter the customer's verified M-Pesa transaction receipt number to confirm payment before publishing live.
                    </p>
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider text-slate-800 mb-1.5">M-Pesa Receipt Number / Transaction Code</label>
                        <input type="text" name="mpesa_transaction_code" value="{{ old('mpesa_transaction_code') }}" required placeholder="e.g. QJK1234567 or RAB9876543" class="w-full px-4 py-3 bg-white border border-amber-300 rounded-xl text-sm font-mono font-bold uppercase text-amber-950 focus:ring-2 focus:ring-amber-500">
                    </div>
                </div>
            @endif
